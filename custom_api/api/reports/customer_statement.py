...
def get_customer_statement():
    ...
    customer = frappe.db.get_value("Customer", customer_id, ["name", "creation"], as_dict=True)
    ...
    summary = frappe.db.sql("""
        SELECT 
            COUNT(si.name) AS total_invoiced,
            COALESCE(SUM(si.grand_total), 0) AS total_invoiced_amount
    ...
    total_invoiced = summary.get("total_invoiced") or 0
    total_invoiced_amount = summary.get("total_invoiced_amount") or 0
    ...
    gl_rows = frappe.db.sql("""
        ...
        ORDER BY posting_date ASC, creation ASC
        LIMIT %s OFFSET %s
    """, (customer_id, page_size, start), as_dict=True)
    ...
    if voucher_map["Sales Invoice"]:
        rows = frappe.db.get_all("Sales Invoice",
            filters={"name": ["in", voucher_map["Sales Invoice"]]},
            fields=["name", "remarks"]
        )
