...
def get_accounts_payable():
    try:
        ...
        filters = frappe._dict(
            {
                ...
                "party_account": _get_arg("payable_account"),
                "party_type": _get_arg("party_type"),
                "party": _get_list_arg("party"),
                "supplier_group": _get_arg("supplier_group"),
                ...
                "group_by_party": 1 if group_by == "supplier" else 0,
        ...
        cache_key = f"custom_api:ap_report:{cache_key_hash}"
        ...
        for row in raw_data:
            ...
            if not row.get("voucher_no"):
                if status_filter or search_term or voucher_type_filters:
                    continue
                if row.get("party") or row.get("supplier_name"):
            ...
            if search_term:
                supplier_str = str(row.get("party", "") or "").lower()
                bill_str = str(row.get("bill_no", "") or "").lower()
                voucher_str = str(row.get("voucher_no", "") or "").lower()
                if (
                    search_term not in supplier_str
                    and search_term not in bill_str
                    and search_term not in voucher_str
                ):
                    continue
            ...
            supplier = row.get("supplier_name") or row.get("party") or "Unknown"
            supplier_totals[supplier] = supplier_totals.get(supplier, 0) + outstanding

            processed_rows.append(
                {
                    "report_date": row.get("posting_date"),
                    "supplier": supplier,
                    "party_type": row.get("party_type"),
                    "payable_account": row.get("party_account"),
        ...
        final_kpis = {
            ...
            "total_suppliers": len(supplier_totals),
            ...
            "bad_debt_risk": kpi_totals["a_91_120"] + kpi_totals["a_121_above"],
            "ageing_summary": {
            ...
            "top_suppliers": [
                {"supplier": s, "outstanding": _format_currency(v)}
        ...
        frappe.log_error(
            message=frappe.get_traceback(), title="Accounts Payable API Error"
        )
        return send_response(
            status="error",
            message=f"Failed to fetch Accounts Payable: {str(e)}",
            status_code=500,
            http_status=500,
        )
