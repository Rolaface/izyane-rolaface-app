...
def _calculate_payable_kpis(rows):
    ...
    for r in rows:
        ...
        supplier = r.get("supplier_name") or r.get("party") or "Unknown"
        supplier_totals.setdefault(supplier, 0)
        supplier_totals[supplier] += outstanding

    avg_payment_days = total_age_days / counted_age_rows if counted_age_rows else 0
    avg_invoice = total_invoiced / total_invoices if total_invoices else 0

    top_suppliers = sorted(supplier_totals.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]
...
def get_accounts_payable():
    ...
    filters = frappe._dict(
        {
            ...
            "party_account": _get_list_arg("payable_account"),
            "party_type": _get_arg("party_type"),
            "party": _get_list_arg("party"),
            "supplier_group": _get_list_arg("supplier_group"),
            "ageing_based_on": _get_arg("ageing_based_on", "Due Date"),
            "calculate_ageing_with": _get_arg("calculate_ageing_with", "Report Date"),
    ...
    for row in raw_data:
        ...
        if row.get("voucher_no"):
            ...
            if search_term:
                supplier = str(row.get("party", "") or "").lower()
                bill_no = str(row.get("bill_no", "") or "").lower()
                voucher_no = str(row.get("voucher_no", "") or "").lower()
                if (
                    search_term not in supplier
                    and search_term not in bill_no
                    and search_term not in voucher_no
                ):
                    continue
        else:
    ...
    kpis = _calculate_payable_kpis(filtered_data)
